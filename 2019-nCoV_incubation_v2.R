...
data <- read_tsv(file = "Line-list.tsv", skip = 1) %>%  
  dplyr::select(`reporting date`, location, gender, age,  
                exposure_start, exposure_end, symptom_onset, 
                `visiting Wuhan`, `from Wuhan`)
nrow(data) # n = 275

data <- data %>% filter(`visiting Wuhan` == 1 & `from Wuhan` == 0)
nrow(data) # n = 111

data <- data %>% filter(!is.na(exposure_end) & !is.na(symptom_onset) & !is.na(exposure_start))
nrow(data) # n = 26
...
tEndExposure[which(tSymptomOnset - tEndExposure == 0)] <- tEndExposure[which(tSymptomOnset - tEndExposure == 0)]-1
